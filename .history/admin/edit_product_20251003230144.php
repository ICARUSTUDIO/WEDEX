...
// Include the Tinify library using a more robust path
$tinify_path = __DIR__ . '/../lib/tinypng/Tinify.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
     if (empty($_POST['name']) || empty($_POST['price']) || empty($_POST['description'])) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            if (function_exists('\Tinify\setKey') && defined('TINIFY_API_KEY') && TINIFY_API_KEY != 'YOUR_API_KEY_HERE') {
                try {
                    // Initialize Tinify
                    ...
                    // If compression is successful, set the new image name for the database
                    $image_name = $new_image_name;

                    // Delete the old image file if it's not the default
                    if ($product['image'] && $product['image'] != 'default.jpg' && file_exists($target_dir . $product['image'])) {
                        unlink($target_dir . $product['image']);
                    }
                } catch(\Tinify\Exception $e) {
                    // If compression fails, fall back to a simple move
                    error_log("TinyPNG Error: " . $e->getMessage());
                    if (move_uploaded_file($source_file, $target_file)) {
                        $image_name = $new_image_name; // Still use the new image
                    } else {
                        $error = "Sorry, there was an error uploading your new file after a compression attempt failed.";
                    }
                }
            } else {
                // Fallback for when API key is not set or library is missing
                if (move_uploaded_file($source_file, $target_file)) {
                    $image_name = $new_image_name; // Use the new image
                } else {
...
<div class="container mx-auto px-4 py-8">
    ...
    <?php endif; ?>
    <?php if (!function_exists('\Tinify\setKey')): ?>
        <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4" role="alert">
            <p><strong>Warning:</strong> The TinyPNG library was not found. Images will be uploaded without compression. Please ensure <code>lib/tinypng/Tinify.php</code> exists.</p>
        </div>
    <?php endif; ?>
