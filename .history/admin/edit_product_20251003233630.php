            ...
            if ($tinify_loaded && defined('TINIFY_API_KEY') && TINIFY_API_KEY != 'YOUR_API_KEY_HERE') {
                try {
                    ...
                    try {
                        ...
                        throw new \Exception("API Key validation failed: " . $e->getMessage() . " (Status: " . $e->status . ")");
                    }
                    ...
                    $compressionCount = \Tinify\Tinify::compressionCount();
                    $_SESSION['compression_status'] = "Success: Image was compressed successfully! (Compressions this month: " . $compressionCount . ")";
                    
                } catch (\Tinify\AccountException $e) {
                    $errorMsg = "Account Error: " . $e->getMessage();
                    if ($e->status == 429) {
                        $errorMsg = "Account Error: You have reached your monthly compression limit.";
                    } elseif ($e->status == 401) {
                        $errorMsg = "Account Error: Invalid API key. Please check your TinyPNG API key in config.php";
                    }
                    $_SESSION['compression_status'] = "Error: " . $errorMsg;
                    // Fallback to regular upload
                    if (!move_uploaded_file($source_file, $target_file)) {
                        $error = "Sorry, there was an error uploading your file after compression failed.";
                        $image_name = $product['image']; // Revert to old image
                    }
                } catch (\Tinify\ClientException $e) {
                    $_SESSION['compression_status'] = "Error: Client error - " . $e->getMessage() . " (Status: " . $e->status . ")";
                    // Fallback to regular upload
                    if (!move_uploaded_file($source_file, $target_file)) {
                        $error = "Sorry, there was an error uploading your file after compression failed.";
                        $image_name = $product['image']; // Revert to old image
                    ...
                    $_SESSION['compression_status'] = "Error: TinyPNG server error - " . $e->getMessage() . " (Status: " . $e->status . ")";
                    // Fallback to regular upload
                    if (!move_uploaded_file($source_file, $target_file)) {
                        $error = "Sorry, there was an error uploading your file after compression failed.";
                        $image_name = $product['image']; // Revert to old image
                    ...
                    $_SESSION['compression_status'] = "Error: Connection failed - " . $e->getMessage();
                    // Fallback to regular upload
                    if (!move_uploaded_file($source_file, $target_file)) {
                        $error = "Sorry, there was an error uploading your file after compression failed.";
                        $image_name = $product['image']; // Revert to old image
                    ...
                    $_SESSION['compression_status'] = "Error: " . $e->getMessage();
                    // Fallback to regular upload
                    if (!move_uploaded_file($source_file, $target_file)) {
                        $error = "Sorry, there was an error uploading your file after compression failed.";
                        $image_name = $product['image']; // Revert to old image
