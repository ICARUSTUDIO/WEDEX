...
// Include the Tinify library
require_once '../lib/tinypng/Tinify.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
     if (empty($_POST['name']) || empty($_POST['price']) || empty($_POST['description'])) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            // Check if API key is set before trying to compress
            if (defined('TINIFY_API_KEY') && TINIFY_API_KEY != 'YOUR_API_KEY_HERE') {
                try {
                    // Initialize Tinify
