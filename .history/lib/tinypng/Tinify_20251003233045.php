...
class Client {
    ...
    public function __construct($key, $appIdentifier = null, $proxy = null) {
        $userAgent = "Tinify/" . Tinify::VERSION . " PHP/" . PHP_VERSION;
        if ($appIdentifier) {
            $userAgent = $appIdentifier . " " . $userAgent;
        }
    ...
    public function request($method, $url, $body = null) {
        ...
        $url = parse_url($url, PHP_URL_SCHEME) ? $url : self::API_ENDPOINT . $url;
        ...
        if ($body !== null) {
            ...
            if (!is_array($body) && strlen($body) > 0) {
        ...
        if (isset($headers["Compression-Count"])) {
            $this->compressionCount = intval($headers["Compression-Count"]);
        }
        ...
        $message = isset($details->message) && isset($details->error)
            ? $details->message . " (HTTP " . $status . "/" . $details->error . ")"
            : "Unknown error";
